    ...
    while (configFile.available()) {
      ...
      if (character == '/' && previousCharacter == '\n') {
        ...
        if (character == '\n') {
          ...
          if (Temp.lastIndexOf(setting) != -1) { // if the setting is in the file
            Serial.print(F("From config.txt: "));
...
String ConvertValuesToKeycodes(String given) {
  ...
  while (given.lastIndexOf("+") > -1) {
    temp2.concat(ConvertValueToKeycode(given.substring(0, given.indexOf("+"))));
    temp2.concat('+');
    given = given.substring(given.indexOf("+") + 1, given.length());
  }
  temp2.concat(ConvertValueToKeycode(given));
  Serial.println(temp2);
  return temp2;
}

String ConvertValueToKeycode(String given) {
  String Temp = "";
  File mappingFile;
  mappingFile = SD.open("TMF.txt");
  //todo don't run this if given is blank
  if (mappingFile) {
    char character;
    char previousCharacter = '\n';
    while (mappingFile.available()) {
      character = mappingFile.read();
      //if comment, ignore the line
      if (character == '/' && previousCharacter == '\n') {
        while (character != '\n') {
          character = mappingFile.read();
        }
      } else {
        //if the line has been read
        if (character == '\n') {
          //temp is a line from the mapping file
          if (given == Temp.substring(0, Temp.indexOf("="))) {
            mappingFile.close();
    ...
    Serial.println(F("error oppening mapping file"));
  }
  mappingFile.close();
  return "w(" + given + ")";
}
